...
public class UpdateProjectServlet extends DBConnectionServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String[] createdDatesArray=request.getParameterValues("createdDate");
		java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
		String[] startDatesArray=null;
		String[] endDatesArray=null;
		List<Assignment> availAssignments=null;
		...
		Project project=dataManager.getProjectDataByEmployer(code,emp.getEid());
			...
			if (employeeIDsArray!=null){
				for (int i=0;i<employeeIDsArray.length;i++){
					...
					if (!startYearArray[i].equals("") && !startMonthArray[i].equals("") && !startDayArray[i].equals("") && !endYearArray[i].equals("") && !endMonthArray[i].equals("") && !endDayArray[i].equals("")){
						...
						try {
							createdDate = (Date) sdf.parse(createdDatesArray[i]);
					...
					//System.out.println("Created date: "+sdf.format(createdDate));
						...
						assignment.setProjectCode(project.getCode());
						availAssignments=dataManager.getProjectAssignmentsByProject(project);
		...
		if((assignmentList==null || assignmentList!=null && valOK) && !code.equals("") && !title.equals("") && !description.equals("")){
											...
											if(valOK && emp.getEid()!=0 && project!=null){
												dbOK=dataManager.updateProject(project);
		...
		{	
			...
			for (Assignment assignment:assignmentList){
				for(Assignment availAssignment:availAssignments){
					if(availAssignment.getProjectCode().equals(assignment.getProjectCode()) && availAssignment.getEmployeeId()==assignment.getEmployeeId() && availAssignment.getCreatedDate().compareTo(assignment.getCreatedDate())==0)
						avail=true;
				}
			if(!avail){
		...
		dispatcher = request.getRequestDispatcher("/projectdetail.jsp");
